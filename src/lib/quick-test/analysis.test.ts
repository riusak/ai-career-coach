...
describe('analyzeResumeText', () => {
  ...
  it('returns a score within bounds and populated findings for a solid CV', () => {
  ...
  it('recognizes quantified achievements even before punctuation (35 %.)', () => {
    ...
    expect(analysis).not.toBeNull();
    expect(
      analysis!.weaknesses.some((weakness) => weakness.includes('résultats chiffrés'))
    ).toBe(false);
  });
  ...
  it('flags missing contact information as a weakness', () => {
    ...
    expect(noContact!.weaknesses.some((weakness) => weakness.includes('Coordonnées'))).toBe(
      true
    );
  });
