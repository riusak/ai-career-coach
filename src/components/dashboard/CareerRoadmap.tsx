...
interface CareerRoadmapProps {
  ...
  onSelectMilestone: (milestone: MilestoneData) => void;
...
/**
 * « Progression de Carrière » — exact port of the template's CareerRoadmap.tsx:
 * ascending orange mountain curve with company badged nodes, hover detail
 * popovers (portal) and the earlier-experiences lead-in indicator.
 */
export default function CareerRoadmap({
  ...
  const handleBadgeMouseEnter = (m: MilestoneData, el: HTMLElement) => {
    clearHoverTimeout();
      ...
      return (
        <div className="relative w-16 h-16 sm:w-18 sm:h-18 flex items-center justify-center select-none">
          ...
          <div className="relative w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-white flex items-center justify-center shadow-[0_8px_20px_rgba(255,122,0,0.25)] ring-4 ring-orange-100/80">
            {/* Ultimate professional goal — the template flag, anchored on the
                explicit career goal set in the profile (migration 012). */}
            <Flag className="w-6 h-6 sm:w-7 sm:h-7 text-[#FF7A00] fill-[#FF7A00]/20" />
          </div>
        </div>
      );
    }
      ...
      {hoveredInfo &&
        ...
        createPortal(
          (() => {
            ...
            const maxLeft =
              (typeof window !== 'undefined' ? window.innerWidth : 1200) - 16 - halfWidth;
